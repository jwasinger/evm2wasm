...
string evm2wast(const string& evmCode, bool stackTrace, bool useAsyncAPI, bool inlineOps)
{
    ...
    auto addStackCheck = [&segment, &segmentStackHigh, &segmentStackLow, &segmentStackDeta]() {
    ...
    for (size_t pc = 0; pc < evmCode.length(); pc++)
    {
        ...
        auto stackDeta = op.on - op.off;
        // update the stack pointer
        if (stackDeta != 0)
        {
            segment << "(set_global $sp(i32.add(get_global $sp)(i32.const {stackDeta})))\n"_format(
                "stackDeta"_a = stackDeta * 32);
        }
...
std::string assembleSegments(const std::vector<JumpSegment>& segments)
{
    ...
    for (size_t index; index < segments.size(); ++index)
    {
